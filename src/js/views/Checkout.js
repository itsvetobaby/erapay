...
class Checkout extends Mesh {
  ...
  changeItemCount(k, v) {
    ...
    State.local.get('cart').get(this.props.mesh).get(k).put(this.cart[k]);
  }
      ...
      <div class="flex-table">
        ${Object.keys(this.cart).filter(k => !!this.cart[k] && !!this.state.items[k]).map(k => {
          ...
          return html`
            <div class="flex-row">
              <div class="flex-cell">
                <a href=${'/point/' + k + '/' + this.props.mesh}>
  ...
  render() {
    ...
    <div class="main-view" id="profile">
      <div class="content">
        <p>
          <a href="/mesh/${this.props.mesh}"><iris-text path="profile/name" user=${this.props.mesh}/></a>
        </p>
        <div id="mesh-steps">
  ...
  componentDidMount() {
    mesh.prototype.componentDidMount.call(this);
